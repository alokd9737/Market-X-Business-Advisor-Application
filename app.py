...
import os
import requests
from da*etime import datetime
from sklearn*feature_extraction.text import Tfi*fVectorizer
from sklearn.metrics.p*irwise import cosine_similarity
...
# =============================*===========================
st.set*page_config(
    page_title="Marke* X Business Advisor",
    page_ico*="💼",
    layout="wide"
)
...
# SAFE SECRET R*ADER
# ===========================*=============================
def *et_secret(key, default=""):
    tr*:
        return st.secrets.get(ke*, default)
    except Exception:
 *      return os.environ.get(key, d*fault)
...
st.markdown(
    """
    <style>*    .main {
        background-col*r: #f8fafc;
    }

    .hero-box {*        background: linear-gradien*(135deg, #0f172a, #1e3a8a);
      * padding: 35px;
        border-rad*us: 20px;
        color: white;
  *     margin-bottom: 25px;
        *ox-shadow: 0px 8px 24px rgba(0,0,0*0.18);
    }
...
*       font-size: 18px;
    ...
    .lead-box {
       ...
       *padding: 22px;
        border-radi*s: 16px;
        box-shadow: 0px 4*x 14px rgba(0,0,0,0.08);
    }

  * .cta-box {
        background-col*r: #eff6ff;
    ...
    .me*ric-card {
        ...
        padding: 18px;
  *     border-radius: 14px;
        *ox-shadow: 0px 3px 12px rgba(0,0,0*0.08);
...
*
# ===============================*=========================
# DEFAUL* KNOWLEDGE BASE
# These files are *reated automatically when app star*s.
# You can later edit/add more .*xt files in the data folder.
# ===*==================================*==================
default_knowled*e = {
    "company_profile.txt": "*"
Market X is a fast-growing consu*ting firm in India focused on FMCG* dairy, agriculture, government pr*jects, brand development, business*development, and market expansion.*
Market X works with corporates, g*vernment bodies, and growth-stage *usinesses looking to enter new mar*ets, improve sales, build distribu*ion networks, conduct market resea*ch, and execute business developme*t projects.

Market X has experien*e across Indian states including B*har, Jharkhand, Uttar Pradesh, Ker*la, Andhra Pradesh, Assam, and Aru*achal Pradesh.

The firm supports *lients through strategic advisory,*market intelligence, execution pla*ning, sales transformation, and business growth consulting.
""",
...
# =========================================================
def fallback_response(query, context, service_area):
    if context:
        return f"""
...
""", "Blocked: Non-business question"
    ...
    fallback = fallback_response(query, context, service_area)
...
with st.sidebar:
    ...
    st.write(
        """
        This assistant is designed for business and consulting-related questions only.
        """
    )
